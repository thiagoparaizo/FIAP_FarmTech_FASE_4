    ...
    def train_models(self, sensor_data):
        ...
        try:
            ...
            X_train, X_test, y_irr_train, y_irr_test, y_hum_train, y_hum_test = train_test_split(
                X_scaled, y_irrigation, y_humidity, test_size=0.3, random_state=42
            )
            ...
            # Validação cruzada
            cv_irrigation = cross_val_score(self.irrigation_classifier, X_scaled, y_irrigation, cv=5)
            cv_humidity = cross_val_score(self.humidity_regressor, X_scaled, y_humidity, cv=5, scoring='neg_mean_absolute_error')
